        ...
        foreach ($this->docentes as $docente):
            ?>
                <div class="docente" id="<?= $docente->cd_docente ?>">
                    ...
                    <button class="desligarDocente" docente="<?= $docente->nome_docente ?>" id="<?= $docente->cd_docente ?>" <?= $docente->id_cargo>=$_SESSION['logged']['cargo']?'disabled':'' ?>>Desligar Docente</button>
                </div>
            <?php
        endforeach;
    ?>
