...
CREATE TABLE IF NOT EXISTS `db_bhaskara`.`tb_nota` (
  ...
  `dt_nota` DATE NOT NULL DEFAULT current_timestamp,
  `id_periodo_letivo` INT NOT NULL,
  `id_matricula` INT(6) NOT NULL,
  `id_turma` INT NOT NULL,
  `id_materia` INT NOT NULL,
  ...
  INDEX `fk_tb_nota_tb_turma_materia1_idx` (`id_turma` ASC, `id_materia` ASC) ,
  ...
  CONSTRAINT `fk_tb_nota_tb_turma_materia1`
    FOREIGN KEY (`id_turma` , `id_materia`)
    REFERENCES `db_bhaskara`.`tb_turma_materia` (`id_turma` , `id_materia`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
